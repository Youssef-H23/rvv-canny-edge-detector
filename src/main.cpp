...
int main(int argc, char* argv[]) {
...
#ifdef USE_RVV
        sobel_gradients_rvv(blurred, gx, gy, width, height);
#else
...
#ifdef USE_RVV
        compute_magnitude_l1_rvv(gx, gy, mag_l1, width, height);
#else
    ...
    double sum_stages = t_gaussian + t_sobel + t_mag_l1 + t_dir + t_nms + t_thresh + t_hyst;
